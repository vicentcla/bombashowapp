-- =============================================================
-- SCRIPT 2 (ejecutar DESPUÉS del SCRIPT 1)
-- Consolidado idempotente: acceso / aprobación / onboarding.
-- Aplica todo lo que falte en cualquier proyecto Supabase y puede
-- ejecutarse varias veces sin romper nada.
-- =============================================================

-- ---------- Esquema privado + helpers ----------
...
-- Correo de un usuario, solo visible por el propio usuario o por admins
...
-- Permisos: el email de profiles NO se expone por columna (solo via get_profile_email)
...
-- ---------- Alta automática de usuario (perfil + rol) ----------
...
-- ---------- Proteger el estado de aprobación ----------
...
-- ---------- Políticas: solicitudes de admin ----------
...
-- ---------- Políticas del contenido (solo si la tabla existe) ----------
DO $$
BEGIN
...
-- Marcar como "onboarded" a los usuarios ya aprobados con nombre (no repetir el alta)
...
-- Garantizar al menos un superadmin
